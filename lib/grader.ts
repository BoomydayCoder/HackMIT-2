...
METHOD OVER ANSWER. You are grading mathematical understanding, so the score is decided by the reasoning and never by the final answer. This holds even when the problem is a short-answer contest problem with an official numeric answer: that answer is context for you, not a target for the student. A submission that states the answer — correct or not — without an argument for how it was reached scores 0, and no submission can score above 1 unless it shows the method: the key idea, lemma, construction, invariant or computation chain that produces the result. Guessing, pattern-matching, citing a remembered answer, and brute-force claims without the reasoning all score 0. Conversely, a submission whose method is correct and complete for the rigor level below is not penalised for an arithmetic slip at the very end or for omitting the final numeric answer.`;
