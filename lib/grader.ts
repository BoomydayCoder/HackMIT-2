...
const RUBRICS: Record<RigorLevel, string> = {
  1: `RIGOR LEVEL 1 — AXIOMATIC. Grade like a formal-verification referee. A 7 requires that every single inference is explicitly justified: every case is enumerated, every inequality is derived rather than asserted, every "clearly", "obviously", "similarly", "WLOG" or "it is easy to see" is either expanded or costs points, and every object used is shown to exist and be well-defined. Named theorems may be cited only with their hypotheses verified explicitly. Score guide: 7 = nothing left to the reader; 5–6 = correct and complete but with one or two small unexpanded steps; 3–4 = correct overall argument with several unjustified but true steps; 1–2 = the right strategy with major gaps; 0 = wrong or no progress. Even a correct, elegant olympiad-style proof should typically land around 4–5 here unless it is unusually explicit. List every unjustified step in "gaps".`,
  2: `RIGOR LEVEL 2 — JOURNAL. Grade as a careful referee for a mathematics journal. A 7 requires a complete, correct proof in which every non-routine step is justified and routine algebra may be summarised but must be verifiable by the reader in a few lines. Well-known theorems may be cited by name provided their hypotheses clearly hold. Deduct for hand-waving on any step that is not genuinely routine, for missing edge cases, and for gaps in case analyses. Score guide: 7 = complete and rigorous; 5–6 = complete with minor imprecision or one small omitted routine step; 3–4 = substantially correct with a real but repairable gap; 1–2 = correct strategy but important parts missing; 0 = wrong or no progress.`,
  3: `RIGOR LEVEL 3 — OLYMPIAD. Grade exactly as an IMO coordinator applying the standard marking scheme. A 7 is a complete and correct proof; routine computations and genuinely easy steps may be skipped, but every essential idea must be present and correctly argued. 5–6 means a complete solution with only minor gaps or slips that do not affect the validity of the argument. 1–4 means substantial partial progress: key lemmas proved, the main construction found, or the problem reduced to an easier one. 0 means no meaningful progress. Do not reward unjustified claims of key steps. For short-answer (AMC-style) problems a bare correct answer with no justification scores at most 1; full marks require a complete, clearly argued solution.`,
  4: `RIGOR LEVEL 4 — SKETCH. Grade the mathematical content, not the write-up. A 7 requires that all the key ideas are present and correct: the crucial lemma(s), the main construction or invariant, and the way they combine to finish the problem. The student may omit computations, routine verifications, straightforward case checks and formal write-up, as long as an expert could complete the proof from the sketch without any new idea. Deduct only for missing or incorrect ideas: 5–6 = all main ideas present but one needed step is asserted without indicating why it holds; 3–4 = the main idea is found but a second essential idea is missing; 1–2 = a relevant observation or partial reduction; 0 = wrong direction or no progress. Be lenient about informal language and notation. For short-answer problems, a correct answer with a brief but correct explanation of the method earns 7.`,
  5: `RIGOR LEVEL 5 — IDEA. Grade whether the student clearly knows how to solve the problem. A 7 requires only that the submission identifies the correct central idea(s) and the correct answer (where one exists), even if expressed vaguely, informally, out of order, or in a single sentence. Do not deduct for missing details, missing justifications, sloppy notation, or missing computations. Deduct only when the idea itself is wrong, when a central idea is missing, or when the description is too vague to distinguish a solver from a guesser: 5–6 = the right idea with a small inaccuracy or one missing ingredient; 3–4 = a good relevant idea that is not enough on its own; 1–2 = vaguely relevant thoughts; 0 = nothing relevant, or a bare answer with no idea behind it. Encourage in feedback: say what the student would need to add to turn the idea into a full proof.`,
};
